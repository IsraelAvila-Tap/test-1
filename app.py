...
def _norm_name(s: str) -> str:
    return _strip_accents(str(s)).lower().replace(" ", "").replace("_","")
...
def _parse_spanish_date(val):
    ...
    if m:
        d = int(m.group(1))
        mon = _strip_accents(m.group(2)).lower()
        y = int(m.group(3))
...
def coerce_spanish_date_column(df: pd.DataFrame, candidates, outcol: str) -> bool:
    """
    Busca la columna de fecha por nombre normalizado (sin acentos/espacios/_),
    la parsea con _parse_spanish_date y la guarda en df[outcol].
    Si no encuentra ninguna candidata, intenta autodetectar la mejor columna objeto.
    """
    # 1) matching estricto por nombre normalizado
    norm_map = { _norm_name(c): c for c in df.columns }
    for cand in candidates:
        target = _norm_name(cand)
        if target in norm_map:
            col = norm_map[target]
            df[outcol] = df[col].apply(_parse_spanish_date)
            return True
    # 2) autodetección: prueba en columnas de texto y elige la de mayor % parseado
    best_col, best_ok = None, -1
    for col in df.columns:
        if df[col].dtype == object:
            parsed = df[col].apply(_parse_spanish_date)
            ok = parsed.notna().mean()
            if ok > best_ok:
                best_ok, best_col = ok, col
                df[outcol] = parsed
    return best_ok >= 0.2  # al menos 20% parseable
...
def load_mlp_scores_from_sheet() -> pd.DataFrame:
    ...
    Retorna columnas: MLP, SVC, SCORE (0..1)
    """
    df = read_sheet(SHEET_ID, SHEET_TABS["arer"])
    out_cols = ["MLP","SVC","SCORE"]
    if df.empty:
        return pd.DataFrame(columns=out_cols)

    find_and_rename(df, ["MLP","Proveedor","Carrier","Partner"], "MLP", False, "AR-ER")
    find_and_rename(df, ["SVC","LC","Facility"], "SVC", required=False, source_label="AR-ER")
    find_and_rename(df, ["Solicitados","Solicitudes","Rutas solicitadas"], "SOL", False, "AR-ER")
    find_and_rename(df, ["Aceptados","Confirmados"], "ACEP", False, "AR-ER")
    find_and_rename(df, ["Cancelados"], "CANC", required=False, source_label="AR-ER")
    find_and_rename(df, ["Ejecutados","Presentados"], "EJEC", False, "AR-ER")

    df = ensure_columns(df, {"MLP":"", "SVC":"", "SOL":0, "ACEP":0, "CANC":0, "EJEC":0})
    _as_str_cols(df, ["MLP","SVC"])
    for c in ["SOL","ACEP","CANC","EJEC"]:
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)

    # Agregado (suma por MLP,SVC)
    g = df.groupby(["MLP","SVC"], dropna=False)[["SOL","ACEP","CANC","EJEC"]].sum().reset_index()

    # Métricas (con seguros de división)
    acept = (g["ACEP"] / g["SOL"].replace(0, np.nan)).fillna(0).clip(0,1)
    ejec  = (g["EJEC"] / (g["ACEP"] - g["CANC"]).replace(0, np.nan)).fillna(0).clip(0,1)
    capef = (g["EJEC"] / (g["SOL"]  - g["CANC"]).replace(0, np.nan)).fillna(0).clip(0,1)

    score = 0.5*ejec + 0.3*acept + 0.2*capef
    out = g[["MLP","SVC"]].copy()
    out["SCORE"] = score.clip(0,1)

    # Fallback por MLP (si hay SVC vacío)
    mlp_only = out.groupby("MLP")["SCORE"].mean().rename("SCORE_MLP").reset_index()
    out = out.merge(mlp_only, on="MLP", how="left")
    out["SCORE"] = out["SCORE"].fillna(out["SCORE_MLP"]).fillna(0.5)
    out.drop(columns=["SCORE_MLP"], inplace=True)
    _as_str_cols(out, ["MLP","SVC"])
    return out[["MLP","SVC","SCORE"]]
...
def load_srm_caps_by_mlp_detailed() -> pd.DataFrame:
    ...
    # tokens
    LV, SV, CAR = ["largev","large","lv","xlv","hb"], ["smallv","small","sv"], ["car","auto"]
    SDD, SPOT, BACK = ["sdd","adenda"], ["spot"], ["back","backup","backlog","bu"]
...
# Ventanas usadas en otros módulos; ajústalas si lo deseas
ROLL_WINDOWS = [14, 30, 90]

def _rolling_shortfall(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ventanas móviles de shortfall (media) y nivel de confirmado.
    Agrupa por SVC×DM×Veh×MLP.
    Crea columnas: SF_MEAN_{w}d, CONF_{w}d
    """
    d = df.sort_values("FECHA").copy()
    grp = d.groupby(["SVC","DELIVERY_MOD","SHP_LG_VEHICLE_TYPE","MLP"], dropna=False)
    for w in ROLL_WINDOWS:
...
def train_shortfall_model(window_days: int = 730) -> FailureModel | None:
    """
    Regresor de SHORTFALL_PCT ponderado por CONF_EFECTIVO.
    """
    try:
        tab_arer = get_tab_name("ar_er", ["AR-ER", "AR ER", "AR_ER", "ARER"])
    ...
    # Features
    num_feats = ["SPR_T2","CONF_EFECTIVO","DOW","IS_WE","MES","SEM"] + \
    ...
    y = pd.to_numeric(ydf["SHORTFALL_PCT"], errors="coerce").fillna(0.0).clip(0, 1)
    w = pd.to_numeric(ydf.get("SF_WEIGHT", 0), errors="coerce").fillna(0.0)
...
def _get_recent_shortfall(arer: pd.DataFrame, days: int = 30,
    ...
    df["FAIL_QTY"] = (df["CONF_EFECTIVO"] - df["EJECUTADO"]).clip(lower=0)
    ...
    agg_pool["SF_RECENT_30_POOL"] = (
        ...
        (agg_pool["CONF_QTY"] + prior_strength).replace(0, np.nan)
    ).fillna(0.0).clip(0,1)
    agg_pool.rename(columns={"CONF_QTY":"CONF_QTY_POOL","N_DAYS":"N_DAYS_POOL"}, inplace=True)

    by_full = agg_full[["SVC","DM_TRAIN","SHP_LG_VEHICLE_TYPE","MLP","SF_RECENT_30","CONF_QTY","N_DAYS"]]
    by_pool = agg_pool[["SVC","DM_TRAIN","SHP_LG_VEHICLE_TYPE","SF_RECENT_30_POOL","CONF_QTY_POOL","N_DAYS_POOL"]]
    return by_full, by_pool
...
def predict_failure(detalles_df: pd.DataFrame,
    ...
    det["Shipments"] = pd.to_numeric(det["Shipments"], errors="coerce").fillna(0).astype(int)
...
class CatEmbShared(nn.Module):
    """
    Embeddings con dimensión compartida para todas las columnas categóricas.
    Devuelve tokens [B, n_cat, emb_dim].
    """
    def __init__(self, indexers: dict, cat_cols: list[str], emb_dim: int):
        super().__init__()
        self.cols = list(cat_cols)
        self.emb_dim = int(emb_dim)
        self.embs = nn.ModuleDict({
            c: nn.Embedding(int(indexers[c]) + 1, self.emb_dim)
            for c in self.cols
        })

    def forward(self, x_cat: torch.Tensor) -> torch.Tensor:
        # x_cat: [B, n_cat] (torch.long)
        toks = [self.embs[c](x_cat[:, i]) for i, c in enumerate(self.cols)]
...
class DL_TabTransformer(nn.Module):
    def __init__(self, indexers, cat_cols, num_dim, d_model=32, nhead=4, nlayers=2, p=0.1):
        ...
        # Embeddings con dimensión COMPARTIDA (= d_model) para TODAS las categorías
        self.embs = nn.ModuleDict({
            c: nn.Embedding(int(indexers[c]) + 1, self.per_token_dim)
        ...
        # Proyección de numéricas al mismo espacio de token
        self.num_proj = nn.Linear(int(num_dim), self.per_token_dim) if num_dim and num_dim > 0 else None

        # Encoder Transformer sobre la secuencia de tokens (categorías + CLS)
        ...
        self.blocks = nn.TransformerEncoder(enc_layer, num_layers=nlayers)

        # Cabeza: concatena [CLS, num_token] si hay numéricas
        in_dim = self.per_token_dim * (2 if self.num_proj is not None else 1)
        self.head = MLP(in_dim, hidden=(128, 64), p=0.2)

    def forward(self, x_num, x_cat_list):
        # x_cat_list: list[LongTensor] con shape [B], una por columna categórica
        # x_num: FloatTensor [B, num_dim] o None si no hay numéricas
        device = self.cls.device

        # Construye tokens categóricos [B, C, d]
        if self.cat_cols and x_cat_list:
            toks = [self.embs[c](x.to(device)) for c, x in zip(self.cat_cols, x_cat_list)]  # cada uno [B, d_model]
            T = torch.stack(toks, dim=1)  # [B, C, d_model]
            B = T.size(0)
        else:
            # Sin categorías: solo CLS
            B = x_num.size(0) if x_num is not None else 1
            T = torch.zeros((B, 0, self.per_token_dim), device=device)

        # Añade CLS delante y pasa por el encoder
        cls_tok = self.cls.expand(B, 1, self.per_token_dim)  # [B,1,d_model]
        X = torch.cat([cls_tok, T], dim=1)                   # [B,1+C,d_model]
        X = self.blocks(X)                                   # [B,1+C,d_model]
        h_cls = X[:, 0, :]                                   # [B,d_model]

        # Token numérico (si existe) y concatenación
        if self.num_proj is not None and x_num is not None:
            num_tok = torch.tanh(self.num_proj(x_num.to(device)))  # [B,d_model]
            h = torch.cat([h_cls, num_tok], dim=1)                 # [B,2*d_model]
        else:
            h = h_cls                                              # [B,d_model]

        # Probabilidad (sigmoid dentro de MLP si así lo definiste)
        return self.head(h)
